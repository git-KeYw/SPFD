...
class DINO_pku(nn.Module):
    def __init__(
        ...
        event_mean: List[float] = [0,0,0],
        event_std:  List[float] = [1,1,1],
        aux_loss: bool = True,
        ...
        self.register_buffer("event_mean", torch.tensor(event_mean).view(-1, 1, 1), persistent=False)
        self.register_buffer("event_std", torch.tensor(event_std).view(-1, 1, 1), persistent=False)
        self.event_normalizer = lambda x: (x - self.event_mean) / self.event_std
    ...
    def forward(self, batched_inputs):
        ...
        (
            ...
            private_feats_r=multi_level_feats_r,
            private_feats_e=multi_level_feats_e,
        )
        ...
        if self.training:
            ...
            if self.vis_period > 0:
                ...
                if storage.iter % self.vis_period == 0:
                    ...
                    masks = {}
                    for k, feat in fused_feats.items():
    ...
    def visualize_training(self, batched_inputs, results):
        ...
        for inp, res in zip(batched_inputs, results):
            img = inp["image"].permute(1, 2, 0).detach().cpu().numpy()  # [H,W,C], float in [0,1]
            img = (img ).clip(0, 255).astype("uint8")

            if getattr(self, "input_format", "RGB") == "BGR":
                img = img[:, :, ::-1]

            org = img.copy()
            if "event" in inp:
                evt = inp["event"].permute(1, 2, 0).detach().cpu().numpy()  # [H,W,C] or [H,W,1]
                if evt.ndim == 3 and evt.shape[2] > 1:
                    evt = evt.sum(axis=2, keepdims=True)
                elif evt.ndim == 2:
                    evt = evt[..., None]
                m = float(np.max(evt)) if evt.size > 0 else 0.0
                if m > 1e-6:
                    evt_u8 = (evt / m * 255.0).clip(0, 255).astype("uint8")
                else:
                    evt_u8 = np.zeros_like(evt, dtype="uint8")
            ...
            if "instances" in inp:
                ...
                gt_boxes = gt_instances.gt_boxes.tensor.numpy() if gt_instances.has("gt_boxes") else np.zeros((0, 4))
                gt_classes = gt_instances.gt_classes.numpy() if gt_instances.has("gt_classes") else np.zeros((0,),
                                                                                                             dtype=int)

                if metadata is not None and hasattr(metadata, "thing_classes"):
                    class_names = metadata.thing_classes
                    gt_labels = [
                        class_names[int(c)] if 0 <= int(c) < len(class_names) else str(int(c))
                        for c in gt_classes
                    ]
                else:
                ...
                v_gt = v_gt.overlay_instances(boxes=gt_boxes, labels=gt_labels if len(gt_labels) else None)
            ...
            vis_img = np.concatenate((anno_img, pred_img, org), axis=1).astype("uint8")  # [H, 3W, 3]
            vis_img = vis_img.transpose(2, 0, 1)  # -> CHW
    ...
    def preprocess_image(self, batched_inputs):
        ...
        images = [self.normalizer(t) for t in images]
        events = [x["event"].to(self.device).float() for x in batched_inputs]
        events = [self.event_normalizer(t) for t in events]

        images = ImageList.from_tensors(images)  # 不传第二个参数 → 默认不做整除约束
        events = ImageList.from_tensors(events) if events is not None else None
    ...
    def freq_coherence_split(
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            Mc = Mc * share_scale + (1 - share_scale) * 0.5
            Mc = Mc.clamp(0, 1)
