...
class TransformerLayerSequence(nn.Module):
    ...
    def __init__(
        ...
        if mi_mechanism:
            multi_branches = [f"layers_for_branch{i}" for i in range(2, inquiry_head_num + 1)]
            for branch in multi_branches:
                setattr(self, branch, nn.ModuleList())
        if isinstance(transformer_layers, nn.Module):
            for _ in range(num_layers):
                self.layers.append(copy.deepcopy(transformer_layers))
                if mi_mechanism:
                    for branch in multi_branches:
                        getattr(self, branch).append(copy.deepcopy(transformer_layers))
        else:
