...
# 自定义 120k 迭代调度器
lr_multiplier_120k = L(WarmupParamScheduler)(
    scheduler=L(MultiStepParamScheduler)(
        values=[1.0, 0.1, 0.01],
        milestones=[80000, 100000, 120000],
    ),
    warmup_length=0 / 120000,   # 前 1000 iter 线性升温
    warmup_method="linear",
    warmup_factor=0.001,
)
TOTAL_ITERS = 140000
WARMUP_ITERS = 1000  # 预热步数，可按需改

lr_multiplier = L(WarmupParamScheduler)(
    scheduler=L(MultiStepParamScheduler)(
        values=[1.0, 0.1],     # 0~80k: ×1.0；80k~140k: ×0.1
        milestones=[80000],
        num_updates=TOTAL_ITERS,
    ),
    warmup_length=WARMUP_ITERS / TOTAL_ITERS,  # 预热占比
    warmup_method="linear",                    # 线性预热
    warmup_factor=0.001,                       # 预热起点 = 0.001 * base LR
)
